...
function rulesetToMap(context, { ruleset: { rules } } = { ruleset: { rules: [] } }) {
  ...
  rules.forEach(rule => {
    ...
    if (!(rule instanceof tree.Declaration)) return;
...
function getBreakpoints(context, breakpoints) {
  if (!breakpoints) {
    if (Object.keys(gridBreakpoints).length === 0) gridBreakpoints = lookupVariable(context, '@grid-breakpoints');
...
functions.add('breakpoint-next', function ({ value: breakpointName }, breakpoints) {
  ...
  if (breakpointIndex === -1) return new tree.Quoted('"');

  // Next breakpoint is null for the last breakpoint.
  if (breakpointIndex + 1 === breakpointNames.length) return new tree.Quoted('"');
...
functions.add('breakpoint-min', function ({ value: breakpointName }, breakpoints) {
  ...
  if (breakpointIndex === 0) return new tree.Quoted('"');
