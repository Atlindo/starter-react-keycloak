...
const Page = () => {
    ...
    return (
        <React.Fragment>
            <PageHeaderAlt>
                <TheContainer fluid={true}>
                    <div className={'py-4'}>
                        <Typography.Title level={1} className={'!m-0'}>Devices</Typography.Title>
                    </div>
                </TheContainer>
            </PageHeaderAlt>
            <TheContainer fluid={true} className={'my-4'}>
                {
                    loading ? <Loading /> :
                        Array.isArray(data) &&
                        data.length > 0 ?
                                            ...
                                            <div
                                                ...
                                                <div className={'!flex items-center justify-between !gap-4'}>
                                                    <Flex align={'center'} gap={10}>
                                                        ...
                                                        <Typography.Title level={3} className={'!m-0'}>{child?.os}</Typography.Title>
                                                        {
                                                            child?.current && (
                                                                            ...
                                                                            <div key={`session-${item?.id}`} className={'p-4 !border rounded-xl space-y-4'}>
                                                                                <Flex align={'center'} justify={'space-between'}>
                                                                                    <Flex gap={10} align={'center'}>
                                                                                        ...
                                                                                        <Typography.Title level={4} className={'!m-0'}>{item?.browser}</Typography.Title>
                                                                                    </Flex>
                                                                                    <Button type={'primary'}>Sign Out</Button>
                                                                                </Flex>
                                                                                <Descriptions>
                                                                                    <Descriptions.Item label={'IP Address'} span={1}>{item?.ipAddress}</Descriptions.Item>
                                                                                    <Descriptions.Item label={'Started'} span={1}>
                                                                                        <Flex gap={5} align={'center'}>
                                                                                            <FieldTimeOutlined/>
                                                                                            <span>{moment(item?.started).isValid() ? moment.unix(item?.started).format('DD MMM YYYY | HH:mm A'): "-"}</span>
                                                                                        </Flex>
                                                                                    </Descriptions.Item>
                                                                                    <Descriptions.Item label={'Last Access'} span={1}>
                                                                                        <Flex gap={5} align={'center'}>
                                                                                            <FieldTimeOutlined/>
                                                                                            <span>{moment(item?.lastAccess).isValid() ? moment.unix(item?.lastAccess).format('DD MMM YYYY | HH:mm A'): "-"}</span>
                                                                                        </Flex>
                                                                                    </Descriptions.Item>
                                                                                    ...
                                                                                    <Descriptions.Item label={'Expires'} span={1}>
                                                                                        <Flex gap={5} align={'center'}>
                                                                                            <FieldTimeOutlined/>
                                                                                            <span>{moment(item?.expires).isValid() ? moment.unix(item?.expires).format('DD MMM YYYY | HH:mm A'): "-"}</span>
                                                                                        </Flex>
                                                                                    </Descriptions.Item>
...

                                                                                    {
                                                                                        ...
                                                                                        <Descriptions.Item label={'Clients'} span={4}>{item?.clients.length > 0 ? item?.clients.map((child)=> child?.clientName).join(' | ') : '-'}</Descriptions.Item>
                                                                                    }
