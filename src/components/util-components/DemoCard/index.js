...
const DemoCard = props => {
  ...
  return (
    <div css={codeBoxCss(token)}>
      <section css={codeBoxDemoCss(token)}>{children}</section>
        ...
        {markdown && (
          <Markdown
            ...
            components={{
              ...
              hr: () => <></>,
              p: p => {
                return typeof p.children[0] === 'string' && p.children[0].match(/[\u4e00-\u9faf]/) ? (
                  ''
                ) : (
                  <p>{p.children}</p>
                );
              ...
              pre: pre => {
                ...
                return <CodeBox language={language}>{props.children}</CodeBox>;
              },
            }}
          />
        )}
